...
def trace_mode(arrival, service, m, setup_time, delayedoff_time, file_index):
...
##    service_time = [0.1, 0,2, 0,3, 0,4]
    ...
    departure_file = 'departure_' + str(file_index) + '.txt'
    mrt_file = 'mrt_' + str(file_index) + '.txt'
    
    with open(departure_file, 'a') as f:
        for i in departure_info:
            f.write(f'{i[0]:.3f}\t{i[1]:.3f}')

    with open(mrt_file, 'a') as fi:
        f.write(f'{response_time_cumulative/num_customer_served:.3f}')
...
def random_mode(arrival, service, m, setup_time, delayedoff_time, time_end, file_index):
...
def simulation(mode, arrival, service, m, setup_time, delayedoff_time, time_end):
    ...
    if mode == 'random':
        random_mode(arrival, service, m, setup_time, delayedoff_time, time_end)

    elif mode == 'trace':
        trace_mode(arrival, service, m, setup_time, delayedoff_time)
